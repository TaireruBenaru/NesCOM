	...
	public class cpu
	{
		...
		public void RunROM(rom SelfROM, ram SelfRAM)
		{
			...
			while (IsRunning)
			{
				...
				string RegisterText = String.Format("PC Register: {0} A Register: {1} X Register: {2} Y Register: {3} SP Register: {4}", PC_Register.ToString("X"), A_Register.ToString("X"), X_Register.ToString("X"), Y_Register.ToString("X"), SP_Register.ToString("X"));
		...
		public void JSRInstruction(byte Instruction)
		{
			...
			string temp = "0x" + BitConverter.ToString(ByteData).Replace("-", string.Empty);
			UInt16 MemoryAddress = Convert.ToUInt16(temp, 16);
			PushToStack16((UInt16)(PC_Register + 2));
			PC_Register = MemoryAddress;
		...
		public void AndImmInstruction(byte Instruction)
		{
			InstructionLength = 2;
		...
		public void PHPInstruction(byte Instruction)
		{
			InstructionLength = 1;
			
			PushToStack((byte)Status_Register);
						
			Debug.WriteLine("PHP - Identifier byte: " + Instruction);
		}
